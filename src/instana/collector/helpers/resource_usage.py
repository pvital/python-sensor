...
def _get_windows_resource_usage() -> ResourceUsage:
    ...
    try:
        ...
        ctx_switches = process.num_ctx_switches() if hasattr(process, 'num_ctx_switches') else None
        
        return ResourceUsage(
            ru_utime=cpu_times.user if hasattr(cpu_times, 'user') else 0.0,
            ru_stime=cpu_times.system if hasattr(cpu_times, 'system') else 0.0,
            ru_maxrss=memory_info.rss // 1024 if hasattr(memory_info, 'rss') else 0,  # Convert to KB to match Unix
            ...
            ru_inblock=io_counters.read_count if io_counters and hasattr(io_counters, 'read_count') else 0,
            ru_oublock=io_counters.write_count if io_counters and hasattr(io_counters, 'write_count') else 0,
            ru_msgsnd=0,  # Not available on Windows
            ru_msgrcv=0,  # Not available on Windows
            ru_nsignals=0,  # Not available on Windows
            ru_nvcsw=ctx_switches.voluntary if ctx_switches and hasattr(ctx_switches, 'voluntary') else 0,
            ru_nivcsw=ctx_switches.involuntary if ctx_switches and hasattr(ctx_switches, 'involuntary') else 0,
        )
    except ImportError:
